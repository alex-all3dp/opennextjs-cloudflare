...
describe("proxy stub", () => {
	...
	test("throws on non-ok response", async () => {
		...
		await expect(
			(stub as unknown as Record<string, () => Promise<unknown>>).badMethod()
		).rejects.toThrow("DO proxy RPC failed");
	});
...
describe("injectDOProxyBindings", () => {
	...
	test("does not overwrite existing DO bindings", () => {
		...
		expect((env as unknown as Record<string, unknown>)["NEXT_TAG_CACHE_DO_SHARDED"]).toBe(
			existingNamespace
		);
	});
