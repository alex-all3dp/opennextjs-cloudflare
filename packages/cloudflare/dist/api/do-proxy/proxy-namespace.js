 ...
 */
function createProxyStub(service, namespaceName, doIdName, locationHint) {
    ...
    return new Proxy({}, {
        get(_target, method) {
            ...
            // Return the id
            if (method === "id")
                return new ProxyDurableObjectId(doIdName);
            // Return the name
            if (method === "name")
                return doIdName;
            ...
            return async (...args) => {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`DO proxy RPC failed: ${namespaceName}.${doIdName}.${method}() -> ${response.status}: ${errorText}`);
                }
 ...
 */
export function createProxyDurableObjectNamespace(service, namespaceName) {
    return {
        idFromName(name) {
 ...
 */
export function injectDOProxyBindings(env) {
